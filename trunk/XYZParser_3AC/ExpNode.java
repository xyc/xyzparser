...
class ExpNode extends SimpleNode {
  ...
  public ExpTreeNode translate(ProgramDescriptor pd, ClassDescriptor ccd, MethodDescriptor cmd){
	  ...
	  ExpNode.currentMethodDescriptor = cmd;
		...
		if(firstNode.toString().equals("ConstantElementNode")){
			...
			root.arrayOff = arrayChildExpNode.translate(pd, ccd, cmd);
			...
			root = childExpNode.translate(pd, ccd, cmd);
		}
			...
			for(int y=0;y<expListNode.jjtGetNumChildren();y++){
				...
				ExpTreeNode param = (ExpTreeNode)expNode.translate(pd, ccd, cmd);
				root.params.add(param.translate());
			}
		...
		if(expNum==i){
			...
			while(i < expNum && oneNode.toString().equals("MultiExpNode")){
				...
				if(rightNode.toString().equals("ConstantElementNode")){
					...
					rightTreeNode.arrayOff = arrayChildExpNode.translate(pd, ccd, cmd);
					...
					rightTreeNode = oneChildExpNode.translate(pd, ccd, cmd);
				}
					...
					for(int x=0;x<expListNode.jjtGetNumChildren();x++){
						...
						ExpTreeNode param = (ExpTreeNode)expNode.translate(pd, ccd, cmd);
						rightTreeNode.params.add(param.translate());
					}
			...
			if(i < expNum){
				...
				for(;i<expNum;i++){
					...
					if(addFirstNode.toString().equals("ConstantElementNode")){
						...
						addRoot.arrayOff = arrayChildExpNode.translate(pd, ccd, cmd);
						...
						addRoot = addFirstChildExpNode.translate(pd, ccd, cmd);
					}
						...
						for(int k=0;k<expListNode.jjtGetNumChildren();k++){
							...
							ExpTreeNode param = (ExpTreeNode)expNode.translate(pd, ccd, cmd);
							addRoot.params.add(param.translate());
						}
					...
					if(oneNode.jjtGetNumChildren()>1){
						...
						for(;j<oneNode.jjtGetNumChildren();j++){
							...
							if(multiRightNode.toString().equals("ConstantElementNode")){
								...
								multiRightTreeNode.arrayOff = arrayChildExpNode.translate(pd, ccd, cmd);
								...
								multiRightTreeNode =  oneMultiChildExpNode.translate(pd, ccd, cmd);
							}
								...
								for(int x=0;x<expListNode.jjtGetNumChildren();x++){
									ExpNode expNode = (ExpNode)expListNode.jjtGetChild(x);
									ExpTreeNode param = (ExpTreeNode)expNode.translate(pd, ccd, cmd);
									multiRightTreeNode.params.add(param.translate());
								}
